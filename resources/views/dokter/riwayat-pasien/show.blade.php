        ...
        <div class="row">
            <div class="col-lg-8 offset-lg-2">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <p><strong>Nama Pasien:</strong> {{ optional($periksa->daftarPoli->pasien)->nama ?? '-' }}</p>
                        <p><strong>No. Antrian:</strong> {{ $periksa->daftarPoli->no_antrian }}</p>
                        <p><strong>Keluhan:</strong> {{ $periksa->daftarPoli->keluhan }}</p>
                        <p><strong>Poli:</strong> {{ optional($periksa->daftarPoli->jadwal->poli)->nama_poli ?? '-' }}</p>
                        <p><strong>Dokter:</strong> {{ optional($periksa->daftarPoli->jadwal->dokter)->nama ?? '-' }}</p>
                                ...
                                <tbody>
                                    @foreach($periksa->detailPeriksas as $index => $detail)
                                        <tr>
                                            ...
                                            <td>Rp {{ number_format(optional($detail->obat)->harga ?? 0, 0, ',', '.') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
